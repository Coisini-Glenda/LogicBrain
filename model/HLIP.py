...
# Import the new visual encoder from paste3
from src.model.encoder.hlip_visual_encoder import (
...
class DEC_CLIPConfig(PretrainedConfig):
    ...
    def __init__(
            ...
            # New parameters for HLIP visual encoder
            vision_encoder_type: str = "singlescan_h2_token2744",
            # Options: singlescan_h2_token2744, multiscan_h2_token588, multiscan_h2_token1176, multiscan_h3_token1176
...
class DEC_CLIP(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Initialize HLIP visual encoder based on config
        if config.vision_encoder_type == "singlescan_h2_token2744":
            self.vision_encoder = vit_base_singlescan_h2_token2744(pretrained=config.vision_pretrained)
        elif config.vision_encoder_type == "multiscan_h2_token588":
            self.vision_encoder = vit_base_multiscan_h2_token588(pretrained=config.vision_pretrained)
        elif config.vision_encoder_type == "multiscan_h2_token1176":
            self.vision_encoder = vit_base_multiscan_h2_token1176(pretrained=config.vision_pretrained)
        elif config.vision_encoder_type == "multiscan_h3_token1176":
            self.vision_encoder = vit_base_multiscan_h3_token1176(pretrained=config.vision_pretrained)
        else:
            raise ValueError(f"Unsupported vision encoder type: {config.vision_encoder_type}")

        self.language_encoder = AutoModel.from_pretrained(
            config.language_model_name_or_path
        )

        # Get the feature dimension from the vision encoder
        # HLIP visual encoders typically output 768-dim features
        vision_output_dim = self.vision_encoder.embed_dim

        self.mm_vision_proj = nn.Linear(
            vision_output_dim, config.hidden_size
        )
        self.mm_language_proj = nn.Linear(
            self.language_encoder.config.hidden_size, config.hidden_size
        )
    ...
    def encode_image(self, image):
        # Use HLIP visual encoder

        image_feats = self.vision_encoder.forward_features(image)      #size: torch.Size([1, 2049, 768])
        

        # Extract CLS token or global pool the features
        if hasattr(self.vision_encoder, 'num_prefix_tokens') and self.vision_encoder.num_prefix_tokens > 0:
            # Use CLS token
