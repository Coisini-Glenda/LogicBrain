...
from src.dataset.clip_dataset import CSVCLIPDataset, CLIPDataset2
from src.model.HLIP import DEC_CLIP, DEC_CLIPConfig
...
class ModelArguments:
    wb_name: Optional[str] = field(default="HLIP")
    language_model_name_or_path: str = field(
        default="output/bert-base-uncased/"
    )
    ...
    vision_encoder_type: str = field(default="singlescan_h2_token2744")
...
class DataArguments:
    data_root: str = field(
        default="brain_origin_data_processed", 
        metadata={"help": "Root directory for brain image data."}
    )
    csv_data_path: str = field(
        default="cap_data/ceshi_report_mode.csv",
        metadata={"help": "Path to CSV data file."},
    )
...
def main():
    ...
    # Make sure model is in the right dtype
    ...
    train_dataset = CSVCLIPDataset(data_args, tokenizer,mode="train")
    eval_dataset = CSVCLIPDataset(data_args, tokenizer, mode="test")
    ...
    hlip_vision_encoder = model.vision_encoder.state_dict()
    torch.save(hlip_vision_encoder, os.path.join(training_args.output_dir, "pretrained_HLIP_ViT.bin"))
